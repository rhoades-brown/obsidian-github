...
export default class GitHubOctokitPlugin extends Plugin {
	...
	async onload() {
		...
		const ribbonIconEl = this.addRibbonIcon('github', 'GitHub Octokit', (_evt: MouseEvent) => {
			// Called when the user clicks the icon.
			new Notice('GitHub Octokit: Ready to sync!');
		});
		...
		statusBarItemEl.setText('GitHub: Ready');
		...
		this.addCommand({
			...
			editorCallback: (_editor: Editor, _view: MarkdownView) => {
				console.log('Syncing current file...');
				new Notice('GitHub Octokit: Syncing current file...');
			}
		});
